...
describe('detectStack', () => {
  it('detects Python from pyproject.toml', async () => {
    ...
    const result = await detectStack('/mnt/c/project')
  ...
  it('returns null on ENOENT', async () => {
    ...
    const result = await detectStack('/mnt/c/nonexistent')
  ...
  it('tries wsl$ fallback when wsl.localhost fails', async () => {
    ...
    const result = await detectStack('/home/user/project')
    expect(result).not.toBeNull()
    expect(result?.badge).toBe('JS')
  ...
  it('accepts Windows paths directly', async () => {
    ...
    const result = await detectStack('C:\\Users\\dev\\project')
    expect(result?.badge).toBe('Rust')
