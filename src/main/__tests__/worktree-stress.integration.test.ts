...
const TEST_REPO = '/home/user/agentdeck-test/git-project'
...
describeIf('Worktree Stress Tests (real git)', () => {
  ...
  it('non-git project returns original path without error', async () => {
    const nonGitMgr = await createWorktreeManager(
      git,
      () => '/home/user/agentdeck-test/non-git-project',
      REGISTRY_DIR,
      WSL_WORKTREE_DIR,
    )
    ...
    expect(r1.isolated).toBe(false)
    expect(r1.path).toBe('/home/user/agentdeck-test/non-git-project')
