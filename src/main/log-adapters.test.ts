...
describe('ClaudeAdapter', () => {
  ...
  it('getLogDirs returns project-specific dir only', () => {
    const dirs = adapter.getLogDirs('/home/user/my-project')
  ...
  it('getLogDirs entry contains path slug', () => {
    ...
    expect(first).toContain('-home-user-my-project')
    expect(first).toMatch(/~\/\.claude\/projects\/-home-user-my-project\/$/)
  })
  ...
  it('getLogDirs uses CLAUDE_CONFIG_DIR from env context', () => {
    ...
    const first = dirs[0]
    if (!first) throw new Error('Expected first dir')
    expect(first).toMatch(/^\/custom\/claude-config\/projects\/-home-user-my-project\/$/)
  ...
  it('matchSession returns true when cwd found in lines', () => {
    const lines = [JSON.stringify({ type: 'summary', cwd: '/home/user/project', ts: 1000 })]
    expect(adapter.matchSession(lines, '/home/user/project', 1000)).toBe(true)
  })

  it('matchSession returns false when cwd not found', () => {
    const lines = [JSON.stringify({ type: 'summary', cwd: '/home/user/other', ts: 1000 })]
    expect(adapter.matchSession(lines, '/home/user/project', 1000)).toBe(false)
  })

  it('matchSession returns false for empty lines', () => {
    expect(adapter.matchSession([], '/home/user/project', 1000)).toBe(false)
  })
})
...
describe('CodexAdapter', () => {
  ...
  it('getLogDirs returns today date dir in YYYY/MM/DD format', () => {
    const dirs = adapter.getLogDirs('/home/user/any')
    expect(dirs).toHaveLength(1)
    const dir = dirs[0]
  ...
  it('getLogDirs uses CODEX_HOME from env context', () => {
    const dirs = adapter.getLogDirs('/home/user/any', { codexHome: '/custom/codex' })
    expect(dirs).toHaveLength(1)
    const dir = dirs[0]
    if (!dir) throw new Error('Expected dir')
  ...
  it('parseUsage extracts cumulative tokens from token_count event', () => {
    ...
    const contextLine = JSON.stringify({
      type: 'turn_context',
      payload: { turn_id: 't1', model: 'gpt-4o', cwd: '/home/user/project' },
    })
  ...
  it('matchSession returns true when cwd found in lines', () => {
    const lines = [JSON.stringify({ type: 'event', cwd: '/home/user/project', ts: 1000 })]
    expect(adapter.matchSession(lines, '/home/user/project', 1000)).toBe(true)
  })

  it('matchSession returns false when cwd not found', () => {
    const lines = [JSON.stringify({ type: 'event', cwd: '/home/user/other', ts: 1000 })]
    expect(adapter.matchSession(lines, '/home/user/project', 1000)).toBe(false)
  })
})
