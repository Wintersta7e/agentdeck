...
describe('inferContextFromModelId', () => {
  describe('bracket tag', () => {
    it('[1m] → 1M', () => {
      expect(inferContextFromModelId('foo[1m]')).toBe(1_000_000)
    })
    it('[2m] → 2M', () => {
      expect(inferContextFromModelId('foo[2m]')).toBe(2_000_000)
    })
    it('[128k] → 128K', () => {
      expect(inferContextFromModelId('foo[128k]')).toBe(128_000)
    })
    it('[1M] case-insensitive', () => {
      expect(inferContextFromModelId('foo[1M]')).toBe(1_000_000)
    })
  })
  describe('trailing dash suffix', () => {
    it('-128k at end', () => {
      expect(inferContextFromModelId('gpt-7o-128k')).toBe(128_000)
    })
    it('-1m at end', () => {
      expect(inferContextFromModelId('some-model-1m')).toBe(1_000_000)
    })
  })
  describe('colon suffix', () => {
    ...
    it(':200k', () => {
      expect(inferContextFromModelId('some-model:200k')).toBe(200_000)
    })
  })
  describe('rejections', () => {
    ...
    it('rejects empty', () => {
      expect(inferContextFromModelId('')).toBeUndefined()
    })
